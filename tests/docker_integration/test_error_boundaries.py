...
Covers:
  ...
  * ``_resolve_container_config_via_gate`` failure modes (gate lookup failures, bad inputs, restrictive fallbacks)
...
class TestResolveContainerConfigFromPermissions:
    ...
    def test_no_workspace_id_and_no_permissions(self):
        ...
        net, mode = _resolve_container_config_via_gate(workspace_id=None, session_permissions=None)
        assert net == "none"
        assert mode == "ro"

    def test_workspace_id_but_no_permissions(self):
        """Workspace ID present but permissions None → restrictive."""
        net, mode = _resolve_container_config_via_gate(workspace_id="ws-1", session_permissions=None)
        # Falls through to defaults since session_permissions is None
        assert net == "none"
        assert mode == "ro"

    def test_no_workspace_id_with_permissions_fallback(self):
        """No workspace ID but permissions dict present → no raw fallback
        survives: the SSOT fail-closes to the most restrictive config."""
        net, mode = _resolve_container_config_via_gate(
            workspace_id=None, session_permissions={"network": "write", "filesystem": "write"}
        )
        # A missing workspace_id no longer grants bridge/rw from raw
        # session permissions; the SSOT fail-closes instead.
        assert net == "none"
        assert mode == "ro"
    ...
    def test_no_workspace_id_with_banned_permissions(self):
        ...
        net, mode = _resolve_container_config_via_gate(
            workspace_id=None, session_permissions={"network": "banned", "filesystem": "read"}
        )
        assert net == "none"
        assert mode == "ro"
