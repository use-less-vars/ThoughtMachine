...
class PydanticModelVisitor(cst.CSTVisitor):
    ...
    def visit_ClassDef(self, node: cst.ClassDef) -> None:
        ...
        if is_pydantic and (not self.current_model):
            self.current_model = {'name': node.name.value, 'line': self._get_line_number(node), 'fields': [], 'docstring': self._get_docstring(node.body)}
    ...
    def visit_AnnAssign(self, node: cst.AnnAssign) -> None:
        ...
        if isinstance(node.target, cst.Name):
            ...
            docstring = None
            field_info = {'name': field_name, 'type': field_type, 'default': default, 'line': self._get_line_number(node), 'docstring': docstring}
    ...
    def visit_Assign(self, node: cst.Assign) -> None:
        ...
        if len(node.targets) == 1 and isinstance(node.targets[0].target, cst.Name):
            ...
            default = self._code_for_node(node.value) if node.value else None
            field_info = {'name': field_name, 'type': None, 'default': default, 'line': self._get_line_number(node), 'docstring': None}
            self.current_model['fields'].append(field_info)
...
class FieldViewer(ToolBase):
    ...
    def _format_json(self, models: List[Dict[str, Any]]) -> str:
        ...
        output = {'file': self.file_path, 'classes': models}
        ...
        for model in models:
            ...
            if not model['fields']:
                ...
                for field in model['fields']:
                    row = [field['name'], field['type'] or '', field['default'] or '', str(field['line'])]
                    if self.include_docstrings:
