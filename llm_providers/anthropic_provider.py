...
class AnthropicProvider(LLMProvider):
    ...
    """
    PRICING = {'claude-3-opus-20240229': {'input': 15.0, 'output': 75.0}, 'claude-3-sonnet-20240229': {'input': 3.0, 'output': 15.0}, 'claude-3-haiku-20240307': {'input': 0.25, 'output': 1.25}, 'claude-3.5-sonnet-20240620': {'input': 3.0, 'output': 15.0}}

    def __init__(self, config: ProviderConfig):
        super().__init__(config)
        self.client = anthropic.Anthropic(api_key=config.api_key, timeout=config.timeout, max_retries=config.max_retries)
    ...
    def chat_completion(self, messages: List[Dict[str, str]], tools: Optional[List[Dict]]=None, **kwargs) -> LLMResponse:
        ...
        try:
            ...
            if os.environ.get('DEBUG_ANTHROPIC'):
                ...
                log('DEBUG', 'llm.anthropic', f'[DEBUG_ANTHROPIC_RAW] Raw API response type: {type(response)}')
                log('DEBUG', 'llm.anthropic', f'[DEBUG_ANTHROPIC_RAW] Raw API response: {raw_str}')
            if os.environ.get('DEBUG_ANTHROPIC'):
                log('DEBUG', 'llm.anthropic', f'[DEBUG_BEFORE_PARSE] Response type: {type(response)}')
                if hasattr(response, '__dict__'):
                    log('DEBUG', 'llm.anthropic', f'[DEBUG_BEFORE_PARSE] Response has __dict__, keys: {list(response.__dict__.keys())}')
                    try:
                        import json
                        resp_json = json.dumps(response.__dict__, default=str, indent=2)
                        if len(resp_json) > 2000:
                            resp_json = resp_json[:2000] + '... (truncated)'
                        log('DEBUG', 'llm.anthropic', f'[DEBUG_BEFORE_PARSE] Response JSON: {resp_json}')
                    except:
                        pass
                elif isinstance(response, dict):
                    log('DEBUG', 'llm.anthropic', f'[DEBUG_BEFORE_PARSE] Response is dict, keys: {list(response.keys())}')
                elif isinstance(response, str):
                    log('DEBUG', 'llm.anthropic', f'[DEBUG_BEFORE_PARSE] WARNING: Response is string, not JSON object: {response[:500]}')
                else:
                    log('DEBUG', 'llm.anthropic', f'[DEBUG_BEFORE_PARSE] Response repr: {repr(response)[:500]}')
            ...
            if os.environ.get('DEBUG_ANTHROPIC'):
                ...
                if hasattr(e, 'response'):
                    try:
                        ...
                        if len(resp_text) > 1000:
                            resp_text = resp_text[:1000] + f'... (truncated, total {len(resp_text)} chars)'
                        log('DEBUG', 'llm.anthropic', f'[DEBUG_ANTHROPIC_ERROR] Exception response: {resp_text}')
                    except:
                        pass
    ...
    def parse_response(self, raw_response: Any, start_time: float) -> LLMResponse:
        ...
        if os.environ.get('DEBUG_ANTHROPIC'):
            log('DEBUG', 'llm.anthropic', f'[DEBUG_PARSE_RESPONSE] Starting parse, raw_response type: {type(raw_response)}')
            if hasattr(raw_response, '__dict__'):
                log('DEBUG', 'llm.anthropic', f'[DEBUG_PARSE_RESPONSE] raw_response has __dict__')
                for key, value in raw_response.__dict__.items():
                    if key == '_response' or key == 'response':
                        continue
                    log('DEBUG', 'llm.anthropic', f'[DEBUG_PARSE_RESPONSE]   {key}: {value}')
            elif isinstance(raw_response, dict):
                log('DEBUG', 'llm.anthropic', f'[DEBUG_PARSE_RESPONSE] raw_response is dict, keys: {list(raw_response.keys())}')
            elif isinstance(raw_response, str):
                log('DEBUG', 'llm.anthropic', f'[DEBUG_PARSE_RESPONSE] raw_response is string (len={len(raw_response)}): {raw_response[:200]}')
            else:
                log('DEBUG', 'llm.anthropic', f'[DEBUG_PARSE_RESPONSE] raw_response repr: {repr(raw_response)[:200]}')
        if isinstance(raw_response, str):
            if os.environ.get('DEBUG_ANTHROPIC'):
                log('DEBUG', 'llm.anthropic', f'[DEBUG_PARSE_RESPONSE] ERROR: API returned string instead of JSON: {raw_response}')
            raise ValueError(f'API returned string instead of JSON response: {raw_response[:200]}')
        if not hasattr(raw_response, 'content'):
            if os.environ.get('DEBUG_ANTHROPIC'):
                log('DEBUG', 'llm.anthropic', f"[DEBUG_PARSE_RESPONSE] ERROR: raw_response missing 'content' attribute")
                log('DEBUG', 'llm.anthropic', f'[DEBUG_PARSE_RESPONSE] raw_response attributes: {dir(raw_response)}')
                if hasattr(raw_response, '__dict__'):
                    log('DEBUG', 'llm.anthropic', f'[DEBUG_PARSE_RESPONSE] raw_response __dict__ keys: {list(raw_response.__dict__.keys())}')
        ...
        for content_block in raw_response.content:
            if content_block.type == 'text':
                ...
                tool_calls.append({'id': cb_id, 'type': 'function', 'function': {'name': name, 'arguments': arguments}})
        usage = {}
        ...
        return LLMResponse(content=content, reasoning=reasoning if reasoning else None, tool_calls=tool_calls if tool_calls else None, usage=usage, raw_response=raw_response, provider='anthropic', model=self.config.model, latency_ms=latency)
