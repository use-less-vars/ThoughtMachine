...
class HistoryProvider:
    ...
    def add_message(self, message: Dict[str, Any]) -> None:
        ...
        log('DEBUG', 'session.history_provider', f"HistoryProvider.add_message called: role={message.get('role')}, session_id={session_id}, history_len={(len(self.session.user_history) if self.session else 'no session')}")
        if DEBUG_CONTEXT:
            prev_timestamp = None
            if self.session.user_history:
                prev_msg = self.session.user_history[-1]
                prev_timestamp = prev_msg.get('created_at')
                log('DEBUG', 'session.history_provider', f'Previous message created_at: {prev_timestamp}')
            current_timestamp = message.get('created_at')
            log('DEBUG', 'session.history_provider', f'Adding message created_at: {current_timestamp}')
            if prev_timestamp and current_timestamp:
                try:
                    ...
                    if prev_dt > current_dt:
                        log('WARNING', 'session.history_provider', f'Timestamp ordering violation! Previous {prev_timestamp} > current {current_timestamp}')
                except (TypeError, ValueError) as e:
                    log('ERROR', 'session.history_provider', f'Failed to parse timestamps: {e}. Previous: {prev_timestamp}, Current: {current_timestamp}')
        if 'seq' not in message:
            message['seq'] = self.session._get_next_seq()
            if DEBUG_CONTEXT:
                log('DEBUG', 'session.history_provider', f"Assigned seq={message['seq']} to message")
